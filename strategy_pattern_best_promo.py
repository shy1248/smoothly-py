...
def best_promotion(order):
    """所有的折扣策略中选择最大的折扣策略"""
    # 添加新折扣策略时必须手动将策略函数放入该列表， 否则best_promorion不会考虑新添加的策略
    # all_promotions = [
    #     fidetity_promotion, bulkitem_promotion, largeorder_promotion
    # ]

    #  使用globals自动找出所有的折扣策略
    all_promotions = [
        globals()[name]
        for name in globals()
        if name.endswith('_promotion') and name != 'best_promotion'
    ]
